...
func TestAccSQSQueue_fifoQueue(t *testing.T) {
	...
	var queueAttributes map[types.QueueAttributeName]string
	resourceName := "aws_sqs_queue.test"
	rName := fmt.Sprintf("%s.fifo", acctest.RandomWithPrefix(t, acctest.ResourcePrefix))
...
func testAccCheckQueueExists(ctx context.Context, t *testing.T, resourceName string, v *map[types.QueueAttributeName]string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
...
func TestAccSQSQueue_waitTimesConfiguration(t *testing.T) {
	ctx := acctest.Context(t)
	var queueAttributes map[types.QueueAttributeName]string
	resourceName := "aws_sqs_queue.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccQueueConfig_waitTimes(rName, 1, 2),
				Check: resource.ComposeAggregateTestCheckFunc(
...
func TestAccSQSQueue_waitTimesConfigurationUpdate(t *testing.T) {
	ctx := acctest.Context(t)
	var queueAttributes map[types.QueueAttributeName]string
	resourceName := "aws_sqs_queue.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccQueueConfig_waitTimes(rName, 3, 5),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckQueueExists(ctx, resourceName, &queueAttributes),
					acctest.CheckResourceAttrRegionalARN(ctx, resourceName, names.AttrARN, "sqs", rName),
					resource.TestCheckResourceAttr(resourceName, names.AttrName, rName),
...
func TestAccSQSQueue_waitTimesConfigurationLocalStack(t *testing.T) {
	ctx := acctest.Context(t)
	var queueAttributes map[types.QueueAttributeName]string
	resourceName := "aws_sqs_queue.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)

	// Skip this test if not running against LocalStack
	// This test is designed to demonstrate LocalStack configuration
	// but will work with regular AWS as well
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:                 func() { acctest.PreCheck(ctx, t) },
		ErrorCheck:               acctest.ErrorCheck(t, names.SQSServiceID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccQueueConfig_waitTimesLocalStack(rName),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckQueueExists(ctx, resourceName, &queueAttributes),
					acctest.CheckResourceAttrRegionalARN(ctx, resourceName, names.AttrARN, "sqs", rName),
					resource.TestCheckResourceAttr(resourceName, names.AttrName, rName),
...
func TestAccSQSQueue_waitTimesConfigurationFIFO(t *testing.T) {
	ctx := acctest.Context(t)
	var queueAttributes map[types.QueueAttributeName]string
	resourceName := "aws_sqs_queue.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccQueueConfig_waitTimesFIFO(rName, 1, 2),
				Check: resource.ComposeAggregateTestCheckFunc(
					testAccCheckQueueExists(ctx, resourceName, &queueAttributes),
					acctest.CheckResourceAttrRegionalARN(ctx, resourceName, names.AttrARN, "sqs", rName),
					resource.TestCheckResourceAttr(resourceName, names.AttrName, rName),
...
func testAccQueueConfig_waitTimes(rName string, createOccurrence, deleteOccurrence int) string {
	return fmt.Sprintf(`
provider "aws" {
  sqs_wait_times {
...
func testAccQueueConfig_waitTimesLocalStack(rName string) string {
	return fmt.Sprintf(`
provider "aws" {
  # LocalStack-like configuration with optimized wait times
  sqs_wait_times {
...
func testAccQueueConfig_waitTimesFIFO(rName string, createOccurrence, deleteOccurrence int) string {
	return fmt.Sprintf(`
provider "aws" {
  sqs_wait_times {
    create_continuous_target_occurrence = %[2]d
